...
public class InfoHelper {
    ...
    public static void addEnergyAndLore(ItemStack stack, List<String> list) {
        if (!isShiftKeyDown()) {
            list.add(MessageFormat.format("{0} {1}{2}{3}{4} {5}{6}",
                    I18n.get("info.sm.hold.txt"),
                    AQUA, ITALIC, I18n.get("info.sm.shift.txt"), RESET,
                    GRAY, I18n.get("info.sm.forDetails.txt")));
        } else {
            addLore(stack, list);
        }
    }
    ...
    public static String[] getLore(ItemStack stack) {
        String unlocalizeLore = stack.getItem().getDescriptionId() + ".lore";
        String rawLore = I18n.get(unlocalizeLore);
    ...
    public static boolean isShiftKeyDown() {
        return GLFW.GLFW_PRESS == GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_SHIFT) ||
                GLFW.GLFW_PRESS == GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_RIGHT_SHIFT);
    }

    public static boolean isCtrlKeyDown() {
        return GLFW.GLFW_PRESS == GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_LEFT_CONTROL) ||
                GLFW.GLFW_PRESS == GLFW.glfwGetKey(Minecraft.getInstance().getWindow().getWindow(), GLFW.GLFW_KEY_RIGHT_CONTROL);
    }

    public static boolean holdShiftForDetails(List<String> list, boolean inverted) {
        if (isShiftKeyDown() == inverted) {
            list.add(I18n.get("info.sc.holdShiftForDetails.txt", AQUA + "" + ITALIC, RESET + "" + GRAY));
        }
    ...
    public static String ITC() {
        return "" + RESET + "" + ChatFormatting.DARK_AQUA;
    }
    ...
    public static String HITC() {
        return "" + RESET + "" + ITALIC + "" + ChatFormatting.GOLD;
    }
